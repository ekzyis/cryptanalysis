...
def encrypt(k: int, text: Text) -> Tuple[Text, int]:
    """Encrypt the message with the given key with Salsa20.

    Encryption is done by XOR'ing the plaintext with the stream generated by the expansion function.
    ...
    return Text(text ^ limit(text.bit, stream_bits, stream), bit=text.bit), iv
